...
export class StripeComponent implements OnInit, AfterViewInit {
  ...
  api_key : string = 'pk_test_Mg3uNo5oXnEXeDKYBOIW3zKI'; // replace me
  ...
  ngAfterViewInit(){
    ...
    paymentRequest.canMakePayment().then((result) => {
      if (result) {
        ...
        paymentRequestElement.mount("#chatloop-paymentRequest");
        /*
        document.querySelector(".chatloop .card-only").style.display = "none";
        document.querySelector(".chatloop .payment-request-available").style.display = "block";
        */
      }
    });
  ...
  onSubmit(e){
    ...
    let name     = this.form.querySelector('#chatloop-name');
    let address1 = this.form.querySelector('#chatloop-address');
    let city     = this.form.querySelector('#chatloop-city');
    let state    = this.form.querySelector('#chatloop-state');
    let zip      = this.form.querySelector('#chatloop-zip');
